...
export default class ThreeMeshControl {
  parentDomId: string;
  ...
  constructor(
    ...
    this.selectAreaStartPointer.x = -Infinity;
    this.selectAreaStartPointer.y = -Infinity;
    ...
    this.selectArea.renderOrder = 1;
    this.selectArea.position.z = -0.2;
    this.selectArea.scale.setScalar(1);
    this.controls.camera.add(this.selectArea);
  }
  ...
  loadModel(file_name: string, onLoad?: () => void) {
    ...
    loader.load(
      ...
      (gltf: GLTF) => {
        ...
        if (onLoad) onLoad();
        if (this.setLoadError) this.setLoadError(false);
  ...
  resetControls(controls: ThreeMeshControls) {
    this.disposeDragControl();
    this.disposeOrbitControl();
  ...
  setControlPoint(
    ...
    if (
      ...
      if (typeof radius !== "undefined")
        targetModel.controlPoint.scale.set(radius, radius, radius);
    }
  }
  ...
  setControlPrimaryDirection(
    ...
    ThreeMeshControl._setControlDirection(
  ...
  _createCameraAndControls(): ThreeMeshControls {
    ...
    const cameraLight = new THREE.PointLight(0x000000, 3.0);
  ...
  _render() {
    ...
    if (this.controls.camera instanceof THREE.PerspectiveCamera) {
      const scale =
        Math.tan((THREE.MathUtils.DEG2RAD * this.controls.camera.fov) / 2) *
        this.selectArea.position.z;
      this.selectArea.scale.set(
        -scale * this.controls.camera.aspect,
        -scale,
        1
      );
